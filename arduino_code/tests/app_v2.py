"""
Medi-Bot Flask Application v2
Medication dispensing robot — scheduling and patient management dashboard.

Database : MariaDB (default) — pre-installed on Ubuntu 24.04
           Set DB_BACKEND = "sqlite" for local development without a server.
Hardware : Arduino connected via USB serial (/dev/ttyACM0).
Platform : Ubuntu 24.04 on Raspberry Pi.

MariaDB setup (one-time, run as root on the Ubuntu machine):
    sudo mariadb
    CREATE DATABASE medi_bot;
    CREATE USER 'medi_user'@'localhost' IDENTIFIED BY 'medi_pass';
    GRANT ALL PRIVILEGES ON medi_bot.* TO 'medi_user'@'localhost';
    FLUSH PRIVILEGES;
    EXIT;
...
# --- Database ---
# "mariadb" → production (Ubuntu 24.04 with MariaDB pre-installed)
# "sqlite"  → development (no server needed, uses a local .db file)
DB_BACKEND = "mariadb"

SQLITE_PATH = "patients.db"

MARIADB_HOST     = "localhost"
MARIADB_PORT     = 3306
MARIADB_USER     = "medi_user"
MARIADB_PASSWORD = "medi_pass"
MARIADB_DATABASE = "medi_bot"
...
# SQLite uses "?" placeholders; MariaDB/MySQL uses "%s"
PH = "?" if DB_BACKEND == "sqlite" else "%s"

# SQLite uses AUTOINCREMENT; MariaDB uses AUTO_INCREMENT
AUTO_INC = "AUTOINCREMENT" if DB_BACKEND == "sqlite" else "AUTO_INCREMENT"
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        for row in seed_patients:
            c.execute(
                f"INSERT INTO patients (name, room, medication, rfid)"
...
def verify_rfid_for_room(room):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute(f"SELECT rfid FROM patients WHERE room = {PH}", (room,))
    result = c.fetchone()
    conn.close()
...
def scheduler_loop():
    ...
    while True:
        ...
        c.execute(f"""
            ...
            WHERE  schedules.delivery_time = {PH}
        ...
        for job_id, room in jobs:
            c.execute(
                f"UPDATE schedules SET status = 'Waiting for RFID' WHERE id = {PH}",
                (job_id,),
            )
            conn.commit()
            ...
            if verify_rfid_for_room(room):
                c.execute(
                    f"UPDATE schedules SET status = 'RFID Correct' WHERE id = {PH}",
                    (job_id,),
                )
                # TODO: trigger dispense command: arduino.write(b"DISPENSE\n")
            else:
                c.execute(
                    f"UPDATE schedules SET status = 'Wrong RFID' WHERE id = {PH}",
                    (job_id,),
                )
...
# Shared nav bar — injected into every page
NAV = """
<nav class="nav">
...
_HEAD = "<html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
...
PAGE_HOME = (
    ...
    <div class="page">
        ...
        <div class="status-bar">
            ...
            {% endif %}
            <span class="chip ok"><span class="dot green"></span>Database: {{ db_backend }}</span>
        </div>
...
def schedule():
    ...
    c = conn.cursor()
    c.execute(
        f"SELECT name, room, medication FROM patients WHERE id = {PH}",
        (patient_id,),
    )
    name, room, medication = c.fetchone()
    c.execute(
        f"INSERT INTO schedules (patient_id, patient_name, room, medication, delivery_time, status)"
...
def add_patient():
    ...
    c = conn.cursor()
    c.execute(
        f"INSERT INTO patients (name, room, medication, rfid) VALUES ({PH},{PH},{PH},{PH})",
...
def edit_patient(patient_id):
    ...
    c = conn.cursor()
    c.execute(
        f"SELECT id, name, room, medication, rfid FROM patients WHERE id = {PH}",
        (patient_id,),
    )
    patient = c.fetchone()
    conn.close()
...
def update_patient(patient_id):
    ...
    c.execute(
        f"UPDATE patients"
        f"   SET name={PH}, room={PH}, medication={PH}, rfid={PH}"
        f" WHERE id={PH}",
...
def delete_patient(patient_id):
    conn = get_db()
    c = conn.cursor()
    c.execute(f"DELETE FROM patients WHERE id = {PH}", (patient_id,))
    conn.commit()
    conn.close()
    return redirect("/patients")
